...
def family_affinity(family: str, req: Requirements, plot: PlotConstraints) -> float:
    ...
    pc = plot.plot_class
    if "NARROW" in pc:
        preferred.update(('LINEAR', 'COMPACT_RECTANGLE'))
    elif pc in ("SMALL_COMPACT", "SMALL_MODERATE"):
        preferred.update(('COMPACT_RECTANGLE', 'CENTRAL_CORE'))
    elif pc in ("MEDIUM_COMPACT", "MEDIUM_MODERATE"):
...
def score_layout(layout: DesignResult, requirements: Requirements,
    ...
    entry = next((r for r in rooms if r.room_id == start), rooms[0] if rooms else None)
    if entry and bedrooms:
