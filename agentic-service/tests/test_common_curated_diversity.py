...
def test_common_configurations_have_three_unique_valid_geometries():
    grouped = defaultdict(list)
    for plan in common_records():
    ...
    for configuration in TARGETS:
        ...
        assert len(plans) == 3
        assert len({plan.geometry_fingerprint for plan in plans}) == 3
        assert len({plan.topology_family for plan in plans}) == 3
        all_fingerprints.extend(plan.geometry_fingerprint for plan in plans)
        ...
        for plan in plans:
            ...
            plot = PlotConstraints(land_size_perches=50, plot_width_ft=120,
                                   plot_length_ft=120, terrain_type="flat")
...
def test_generate_another_returns_a_distinct_common_geometry(configuration, monkeypatch):
    bedrooms, bathrooms, floors = configuration
    target_plans = [plan for plan in common_records()
    ...
    plot = {"plot_width_ft": 120, "plot_length_ft": 120}
